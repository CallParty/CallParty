...
function callToActionPart1Convo(bot, user, message) {
  syncBotReply(bot, message,
    ...
    syncBotReply(bot, message, `${user.convoData.issueMessage}. ` +
      `You can find out more about the issue here ${user.convoData.issueLink}.`)
  ).then(
    syncBotReply(bot, message, stripIndent`
    ...
    syncBotReply(bot, message, 'Give me a thumbs up once you’ve tried to call!')
  ).then(
...
function callToActionPart2Convo(bot, user, message) {
  ...
  syncBotReply(bot, message, msg_attachment).then(
...
function callToActionPart3Convo(bot, user, message) {
  if (['I left a voicemail', 'I talk to a staffer'].indexOf(message.text) >= 0) {
    ...
    syncBotReply(bot, message, {
      ...
      syncBotReply(bot, message, 'Woo thanks for your work! We’ve had [callCount] calls so far. ' +
        'We’ll reach out when we have updates and an outcome on the issue.')
    ).then(
      syncBotReply(bot, message, 'Share this action with your friends to make it a party [link]')
    ).then(
...
function thanksForSharingConvo(bot, user, message) {
  syncBotReply(bot, message, 'Thanks for sharing! We’ll reach back out if we can be helpful.').then(
